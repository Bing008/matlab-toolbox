function [outData]=Opt_PlotFASToutput(FASTfiles,FASTfilesDesc,ReferenceFile,Channels,ShowLegend,CustomHdr,PlotPSDs,OnePlot)
%..........................................................................
...
%                 is false (many plots).
% SampleRate    - scalar that determines how to sample the original data
%                 based on time. Default is 1 (all the data will be plotted)
%                  When this value is set to 4, one data point will be 
%                  extracted every four time points for plotting.
...
data2 = cell(numFiles, 1);
for iFile = 1:numFiles
    slice_start = find(abs( data{iFile}(:,1) - time_start ) < 0.0001);
    slice_end = find(abs( data{iFile}(:,1) - (time_start + time_slice) )< 0.0001);
    if(isempty(slice_start) || isempty(slice_end))
        disp('Error: time slice requirment is not found in the data file');
    end
    data2{iFile}(:,:) = data{iFile}(slice_start:slice_end, :);
end
data = data2;
%% -------------------------------------------------------------------
SampleRate = 1;
% sample the original data
data3 = cell(numFiles, 1);

for iFile = 1:numFiles
    row_index = 1;
    for sample_i = 1:(SampleRate):length(data{iFile}) % length returns the row number defaltly.
        data3{iFile}(row_index, :) = data{iFile}(sample_i ,:);
